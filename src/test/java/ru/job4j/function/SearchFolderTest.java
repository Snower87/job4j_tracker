package ru.job4j.function;

import org.junit.Test;

import java.util.List;
import java.util.function.Predicate;

import static org.hamcrest.CoreMatchers.is;
import static org.junit.Assert.*;

public class SearchFolderTest {
    @Test
    public void whenFilterSize() {
        List<Folder> list = List.of(
                new Folder("fix", 110),
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        List<Folder> expected = List.of(new Folder("fix", 110));
        Predicate<Folder> pred = f -> f.getSize() > 100;
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }

    @Test
    public void whenFilterName() {
        List<Folder> list = List.of(
                new Folder("fix", 110),
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        List<Folder> expected = List.of(
                new Folder("bug", 75),
                new Folder("bug", 90)
        );
        Predicate<Folder> pred = f -> f.getName().contains("bug");
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }

    @Test
    public void whenFilterName123() {
        List<Folder> list = List.of(
                new Folder("Image1", 10),
                new Folder("Image2", 11),
                new Folder("Image3", 12),
                new Folder("Image4", 13),
                new Folder("Image123", 120),
                new Folder("Image123", 130),
                new Folder("Image123", 140)
        );
        List<Folder> expected = List.of(
                new Folder("Image123", 120),
                new Folder("Image123", 130),
                new Folder("Image123", 140)
        );
        Predicate<Folder> pred = f -> f.getName().contains("123");
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }

    @Test
    public void whenFilterNotContainName123() {
        List<Folder> list = List.of(
                new Folder("Image1", 10),
                new Folder("Image2", 11),
                new Folder("Image3", 12),
                new Folder("Image4", 13),
                new Folder("Image123", 120),
                new Folder("Image123", 130),
                new Folder("Image123", 140)
        );
        List<Folder> expected = List.of(
                new Folder("Image1", 10),
                new Folder("Image2", 11),
                new Folder("Image3", 12),
                new Folder("Image4", 13)
        );
        Predicate<Folder> pred = f -> !f.getName().contains("123");
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }

    @Test
    public void whenFilterNotContainName123AndSizeLess12() {
        List<Folder> list = List.of(
                new Folder("Image1", 10),
                new Folder("Image2", 11),
                new Folder("Image3", 12),
                new Folder("Image4", 13),
                new Folder("Image123", 120),
                new Folder("Image123", 130),
                new Folder("Image123", 140)
        );
        List<Folder> expected = List.of(
                new Folder("Image1", 10),
                new Folder("Image2", 11)
        );
        Predicate<Folder> pred = f -> !f.getName().contains("123") && f.getSize() < 12 ;
        List<Folder> rsl = SearchFolder.filter(list, pred);
        assertThat(rsl, is(expected));
    }
}